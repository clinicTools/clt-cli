...
module.exports = {
  async default(args) {
    ...
    if (package.base === undefined) {
      console.log(
        chalk`\n{bold.red Sorry, <<base>> is not set in package.json. This is a clt project for real?}\n`
      );
      return;
    }
    ...
    await fs.promises.rmdir(path.join(cltDir, "/build/app/src"), {
      recursive: true,
    });
    await fs.promises.mkdir(path.join(cltDir, "/build/app/src"), {
      recursive: true,
    });

    await fs.promises.rmdir(path.join(cltDir, "/build/data/src"), {
      recursive: true,
    });
    await fs.promises.mkdir(path.join(cltDir, "/build/data/src"), {
      recursive: true,
    });
